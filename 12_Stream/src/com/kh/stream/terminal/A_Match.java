...
//		a.method1();
	...
	public void method2() {
		...
		result = students.stream().allMatch(stu -> stu.equals("여자") && stu.getAge() >= 10);
//		result = students.stream().filter(stu -> stu.getAge() >=10).allMatch(stu->stu.getGender().equals("여자");
		System.out.println(result);
//		result = students.stream().anyMatch(stu -> stu.equals("남자") && ((stu.getEnglish()+stu.getMath())/2) >=70);
		result = students.stream().filter(stu -> stu.getGender().equals("남자"))
				.anyMatch(stu -> stu.getMath() + stu.getEnglish() / 2>=70);
		System.out.println(result);
