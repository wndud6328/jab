 ...
 * - Iterator 처럼 일회용 (필요하면 다시 스트림 생성)
 * - 내부 반복 처리
 * 
 * 스트림의 종류
 ...
 *   
 * Stream API
 * - 스트림은 자바 8부터 추가된 기능으로 컬렉션(배열)의 저장 요소들을
 *   하나씩 참조해서 람다식으로 처리할 수 있도록 해주는 반복자
 * - 내부 반복자를 사용해서 중간처리(intermediate), 최종 처리 (terminal) 작업을 수행
...
//		c.method2();
	...
	public void method1() {
		// 첫번째 매개 값 부터 매개 값 이전 까지의 값을 
		// 요소로 가지는 스트림 객체를 생성
		IntStream stream = IntStream.range(1, 10); 
		int sum = stream.sum();
		System.out.println("range의 sum : " + sum); //1~9 합
		
		// 첫번째 매개값 ~ 두 번째 매개값 까지의값을
		// 요소로 가지는 스트림 객체를 생성
		stream = IntStream.rangeClosed(1, 10);
		sum = stream.sum();
		System.out.println("rangeClosed의 sum : "+ sum); // 1~10 합
	}
	/*
	 * 배열로부터 스트림을 생성하는 방법
	 * - Arrays.steam(배열) 메서드로 스트림을 생성한다.
