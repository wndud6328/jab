 ...
 * 패키지가 선언되지 않는 클래스는 자동적으로 이름없는 패키지(default)에 속하게 된다. 
*/
package com.kh;
/*
 * 클래스(class)
 *  - 자바에서 모든 코드는 반드시 클래스 안에 존재해야 하며
 *    서로관련된 코드들을 그룹으로 나누어 별도의 클래스를 구성한다.
 *    클래스들이 모여 하나의 Java 프로그램을 구성한다. 
 */

public class Hello { //클래스 명은 클래스 파일명과 동일해야 한다.
	...
	//실행 메소드(main 메소드) : 자바 프로그램 실행 시 java.exe에 의해 가장 먼저 호출한다.
 ...
 *  - 클래스명 : 대문자로 시작, 여러 개의 단어가 있을 때는 단어 일자리마다 대문자로 표기
 ...
 *   - 운영체제에 독립적이다. (JVM)
 *   - 네트워크와 분산처리를 지향한다. (Java API)
 *   - 멀티 스레드를 지향한다. (Java API)
 *   - 동적 로딩을 지원한다.
 *   
 *   JDK > JRE > JVM
 *   - JDK(Java Development Kit) : 자바 개발 도구
 *    = JRE + 실행파일(in / javac.exe, java.exe 등)  
 *   
 *   - JRE(Java Runtime Environment) : 자바 실행 환경
 *    = JRE + Java API(lib/src.zip)
 *    
 *   - JVM(Java Virtual Machine) : 자바 가상 머신
 *    
 *   * 자바 프로그램 실행 단계
 *    - 확장자가 .java 인 소스파일을 작성한다.
 *    - 작성된 소스파일을 자바 컴파일러(javac.exe)로 컴파일하여
 *    - 확장자가 .class인 바이트코드(기계어)로 번역한다.
 *    - 번역된 바이트코드(기계어)를 JVM의 자바 인터프리터(java.exe)를 이용해 설정한다.
 *    
* */ 
